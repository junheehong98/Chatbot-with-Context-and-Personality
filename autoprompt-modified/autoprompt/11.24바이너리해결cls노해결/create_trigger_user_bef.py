...
class PredictWrapper:
    ...
    def __call__(self, model_inputs, trigger_ids):
        ...
        # 디버깅 로그 추가
        print(f"[DEBUG] model_inputs['input_ids'] shape: {model_inputs['input_ids'].shape}")
        print(f"[DEBUG] trigger_mask shape: {trigger_mask.shape}")
        print(f"[DEBUG] input_ids shape: {model_inputs['input_ids'].shape}")

        print(f"[DEBUG] trigger_ids shape: {trigger_ids.shape}")
        
        logits, *_ = self._model(**model_inputs)
        
        # 디버깅: logits 크기 확인
        print(f"[DEBUG] logits shape: {logits.shape}")
        expected_elements = logits.size(0) * self.num_labels * 2
        assert logits.numel() == expected_elements, \
            f"Logits size mismatch. Expected {expected_elements}, got {logits.numel()}"
...
def replace_trigger_tokens(model_inputs, trigger_ids, trigger_mask):
    ...
    # 추가: 크기 확인
    assert trigger_mask.size(0) == input_ids.size(0), \
        f"Batch size mismatch: trigger_mask {trigger_mask.size(0)}, input_ids {input_ids.size(0)}"
    assert trigger_mask.sum() == trigger_ids.numel(), \
        f"Token count mismatch: trigger_mask {trigger_mask.sum()}, trigger_ids {trigger_ids.numel()}"
    ...
    try:
        ...
        logger.error(f"masked_scatter failed: {e}")
        filled = input_ids
...
def create_label_combinations():
    """2^5 가지의 레이블 조합을 생성합니다 (각 레이블은 0, 1로 저장)."""
    label_values = [0, 1]  # 레이블은 0 또는 1만 가능
    return list(itertools.product(label_values, repeat=5))


def validate_personality(personality):
    """사용자가 제공한 성격 조합이 유효한지 확인."""
    if len(personality) != 5 or any(p not in [0, 1] for p in personality):
        raise ValueError("Invalid personality combination. Must be a list of 5 integers (0 or 1).")


def filter_dataset_by_personality(dataset, personality):
    """성격 조합에 따라 데이터셋 필터링"""
    filtered_dataset = [row for row in dataset if tuple(row[1:]) == tuple(personality)]
    if not filtered_dataset:
        logger.warning(f"No data found for personality combination: {personality}. Using the full dataset.")
        return dataset
    return filtered_dataset



def run_model(user_prompt, personality, args=None):
    if args is None:
        ...
        args = argparse.Namespace(
            train=Path('labeled_dialogues.csv'),
            dev=Path('labeled_dialogues_val.csv'),
            template="<s> {input_text} [T] [T] [T] [P] . </s>",
            num_cand=100,
            ...
            seed=42,
            start_index=0,
            personality=(0, 0, 0, 0, 0),  # 디폴트 성격 조합
        )
        '''
    ...
    # DataLoader 생성
    dataset = [(model_inputs, labels)]
    train_loader = DataLoader(dataset, batch_size=args.bsz)
    dev_loader = DataLoader(dataset, batch_size=args.eval_size)
    ...
    # 특정 성격 조합만 처리
    logger.info(f'Processing personality combination: {args.personality}')
    combination = args.personality
...
if __name__ == '__main__':
    ...
    parser.add_argument('--user-prompt', type=str, required=True, help='사용자 입력 프롬프트')
    parser.add_argument('--personality', nargs=5, type=int, required=True,
                        help='성격 조합 (예: 1 0 1 0 1)')
    ...
    # run_model 함수 호출
    final_prompt = run_model(args.user_prompt, args.personality, args)
