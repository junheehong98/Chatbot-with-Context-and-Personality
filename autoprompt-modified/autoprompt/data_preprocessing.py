...
def preprocess_data(input_file):
    ...
    # 레이블을 정수 리스트로 변환
    data['labels'] = data.apply(lambda row: [
        int(row['Negative/Positive']), 
        int(row['Aggressive/Peaceful']), 
        int(row['Conservative/Open-minded']), 
        int(row['Introverted/Extroverted']), 
        int(row['Unstable/Confident'])
    ], axis=1)
    ...
    train_data[['input_text', 'labels']].to_csv('train.tsv', sep='\t', index=False, header=True)
    dev_data[['input_text', 'labels']].to_csv('dev.tsv', sep='\t', index=False, header=True)

    # CSV 파일로도 저장
    train_data[['input_text', 'labels']].to_csv('train.csv', index=False, header=True)
    dev_data[['input_text', 'labels']].to_csv('dev.csv', index=False, header=True)
